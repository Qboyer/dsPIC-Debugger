			...
			xhr.onreadystatechange = function() {//Call a function when the state changes.
				if(this.readyState == XMLHttpRequest.DONE && this.status == 200) {
					...
					for (var i = 0; i < subString.length; i++) {
						...
						else if(subString[i].charAt(0) == 'f'){	//erase lidar points
							subString[i]= subString[i].slice(2);	//remove f=
							
							var subSubString = subString[i].split(";");
							var id = Number(subSubString[0]);
							var value = Number(subSubString[1]);
							updateDebugValue(id,value);
						}
						else if(subString[i].charAt(0) == 'g'){	//erase lidar points
							subString[i]= subString[i].slice(2);	//remove f=
							
							var subSubString = subString[i].split(";");
							var id = Number(subSubString[0]);
							var name = subSubString[1]
							updateDebugName(id,name);
						}
					}
...
function addRowDebug(name,value){
    // Récupération d'une référence à la table
    var tab = document.getElementById('table-debug');
...
function updateDebugValue(id,value){
    // Récupération d'une référence à la table
    var tab = document.getElementById('table-debug');
    tab.rows[id].cells[1].textContent = value/1000
}function updateDebugName(id,name){
    // Récupération d'une référence à la table
    var tab = document.getElementById('table-debug');
    tab.rows[id].cells[0].textContent = name
}
